...
function sync(a, b, aSyncStates, bSyncStates, apeerId, bpeerId) {
  const MAX_ITER = 200
  let aToBmsg = null; let bToAmsg = null; let i = 0
  do {
...
describe('Automerge syncs Counter objects between peers', function () {
  ...
  describe('Signal Counter', function () {
    it('should be instantiated with start value', () => {
      const state = Automerge.initSignalCounter(
        'init value',
        'counterVal',
        1
      )
    ...
    it('should increment', () => {
      let state = Automerge.initSignalCounter(
        'init value',
        signalName,
        1
      )
  ...
  describe('Sync two peers', function () {
    ...
    describe('update my state', function () {
      ...
      it('should not reply if we have no data as well', () => {
        ...
        const syncMessages = Automerge.generateSyncMessages(
          nextState,
          syncStates,
          peerId
        )
      ...
      it('should sync peers where one has commits the other does not', () => {
        for (let i = 0; i < 10; i++) {
          state = Automerge.incrementSignalCounter(state, `incrementing for the ${i} time`, signalName, 1)
        }

        assert.notDeepStrictEqual(otherPeerState, state)
        const [nextState, otherPeerNextState] = sync(state, otherPeerState, syncStates, otherPeerSyncStates, peerId, otherPeerpeerId)
      ...
      it('should work with prior sync state', () => {
        ...
        [state, otherPeerState] = sync(state, otherPeerState, syncStates, otherPeerSyncStates, peerId, otherPeerpeerId)
    ...
    describe('When peer joins', function () {
      ...
      it('Should get the current state from other peer', () => {
        const [_, otherPeerNextState] = sync(state, otherPeerState, syncStates, otherPeerSyncStates, peerId, otherPeerpeerId)
    ...
    describe('When peer rejoins the call', function () {
      beforeEach(() => {
        ...
        for (let i = 0; i < 20; i++) {
          otherPeerState = Automerge.incrementSignalCounter(otherPeerState, `incrementing for the ${i} time`, signalName, 1)
        }
  ...
  describe('Sync three peers', function () {
    ...
    describe('When third peer joins', function () {
      beforeEach(() => {
        ...
        [state, secondPeerState] = sync(state, secondPeerState, syncStates, secondPeerSyncStates, peerId, secondPeerpeerId)
