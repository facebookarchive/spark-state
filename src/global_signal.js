...
export async function createGlobalSignal(signal, startValue, signalName, guaranteeStateKey, updateState) {
  ...
  if (allPeers.length === 0 && signal.getSignalType() === SIGNAL_TYPE.ARRAY) {
    signal.setIsSynced(true)
  }
  const syncStates = {}
  ...
  channel.onMessage.subscribe((msg) => {
    ...
    if (patch) {
      if (signal.getSignalType() === SIGNAL_TYPE.ARRAY) {
        signal.setIsSynced(false)
      }
    ...
    if (signal.getSignalType() === SIGNAL_TYPE.ARRAY) {
      const allSynced = Object.values(syncStates).every(peerState => peerState.allChangesSent === 1)
      signal.setIsSynced(allSynced)
    }
  })
