 ...
 */
export async function createGlobalArray(startValue, arrayName) {
  if (!Array.isArray(startValue)) {
    throw new TypeError("startValue must be an array");
  }

  const array = await SignalsWrapper.Array(startValue, arrayName)

  const updateState = (state, arrayName, event) => {
    switch (event.updateType) {
      case ARRAY_UPDATE.APPEND:
      ...
      case ARRAY_UPDATE.BATCHED:
        return SparkAutomergeWrapper.updateArrayWithBatchedChanges(
          state,
          'Upload batched changes',
          arrayName,
          event.batchedChanges
        )
    }
  }
