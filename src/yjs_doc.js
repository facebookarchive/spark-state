...
export async function createYDoc(name) {
  ...
  const stateVectorsChannel = Multipeer.getBinaryMessageChannel(
    name + "_statevectors"
  );
  ...
  function broadcastChangesToTargetPeer(peerId) {
    ...
    const encodedMessage = encoding.encodeMessage(
      { s: myId, t: peerId },
      stateUpdate
    );
  ...
  function broadcastStateVectorToPeers() {
    stateVectorsChannel.sendMessage(
      encoding.encodeMessage({ s: myId }, stateVectors[myId])
    );
  }
